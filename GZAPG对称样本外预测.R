...
# ---- 5. 参数初始化和边界 ----
init_params <- c(omega = 0.5, alpha = 0.1, beta = 0.1, gamma = -0.5, a = 0.6, delta = 0.9)
...
for (test_size in 1:50) {
  ...
  p_train <- numeric(train_size)
  lambda_train[1] <- ifelse(1 - beta != 0, omega / (1 - beta), 1e-8)
  
  for (t in 2:train_size) {
    term1 <- abs(train_data[t - 1]) - gamma * train_data[t - 1]
    if (term1 < 0) term1 <- 1e-8
    lambda_train[t] <- (omega + alpha * term1^delta + beta * lambda_train[t - 1]^delta)^(1/delta)
    lambda_train[t] <- max(lambda_train[t], 1e-8)
    
    p_train[t] <- (sqrt(phi^2 + 4 * lambda_train[t]) - phi) / (2 * lambda_train[t])
    p_train[t] <- min(max(p_train[t], 1e-8), 1 - 1e-8)
  }
