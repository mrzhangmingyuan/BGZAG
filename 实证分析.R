...
# ---- 5. 参数初始化和边界 ----
init_params <- c(omega = 0.5, alpha = 0.1, beta = 0.3, gamma = -0.5, a = 0.6, delta = 0.9)
...
for (t in 2:n) {
  ...
  lambda[t] <- (result$par[1] + result$par[2] * term1^result$par[6] + 
                  result$par[3] * lambda[t - 1]^result$par[6])^(1/result$par[6])
...
for (i in 1:n) {
  ...
  lambda_pred[i] <- (result$par[1] + result$par[2] * term1^result$par[6] + 
                       result$par[3] * lambda_prev^result$par[6])^(1/result$par[6])
...
for (t in 2:n) {
  ...
  lambda_hat[t] <- (result$par[1] + result$par[2] * term1^result$par[6] + 
                      result$par[3] * lambda_hat[t - 1]^result$par[6])^(1/result$par[6])
